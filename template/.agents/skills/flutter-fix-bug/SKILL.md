...
4. Create the next work item with type `bug` and state `DISCOVERY`.
...
9. Present one combined Playback in `spec.md` covering expected and observed
   behavior, reproduction evidence, root cause, minimal repair, neighboring
   callers, regression scenario, preserved architecture, packages/platforms,
   and verification.
10. Set `PLAYBACK_READY`; wait for explicit approval and record
    `PLAYBACK_APPROVED`.
11. Create a decision-complete minimal `plan.md`. Set `PLAN_READY`; wait for
    explicit approval and record `PLAN_APPROVED`.
...
15. Write `result.md` with before/after evidence and set `VERIFIED` only when
    required checks pass; otherwise set `BLOCKED`.
